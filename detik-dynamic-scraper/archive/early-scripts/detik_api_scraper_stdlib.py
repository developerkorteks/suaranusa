#!/usr/bin/env python3
"""
Detik.com API Scraper - Complete Flow Analysis (Standard Library Only)
...
class DetikAPIScraper:
    ...
    def _fetch_url(self, url, is_json=False):
    ...
    def get_recommendation_articles(
        ...
        if not data:
            return {"articles": [], "total": 0, "error": "Failed to fetch"}
    ...
    def get_recommendation20_articles(self, size=50, user_id="test") -> Dict[str, Any]:
        """
        Mengambil artikel dari API recommendation20 (versi lain - untuk video)
        """
    ...
    def scrape_terpopuler(self, days=None) -> Dict[str, Any]:
        ...
        for url_found, content in matches:
            ...
            # Extract ID dari URL
            id_match = re.search(r"/d-(\d+)/", url_found)
                ...
                articles.append(
                    {
                        ...
                        "title": title,
                        "url": url_found,
                        "source": f"terpopuler_{days}" if days else "terpopuler_all",
                    }
                )

        print(f"[✓] Found {len(articles)} articles from terpopuler")
        return {"articles": articles, "total": len(articles), "unique_ids": article_ids}
...
def main():
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            "  - User ID for API: Can be random string (e.g., 'random.id.here', 'test')\n"
        )
        f.write("  - Video ID format: {YYMMDD}{sequential number} (e.g., 260323133)\n")
